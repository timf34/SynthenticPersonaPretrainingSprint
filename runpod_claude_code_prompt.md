...
3. Launch in tmux, watch the first ~10 minutes of logs (generation throughput, no OOM — tune `--batch_size` in step 2 for an 80GB card, start at 64 for a 3B model and back off if needed), then leave it.
