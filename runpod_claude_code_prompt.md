...
2. **Phase 2 (overnight):** For gate-passing models, run the full assistant-axis pipeline with **90 personas** (the paper used 275) and check via PCA whether PC1 of the persona-vector space aligns with the Assistant Axis, with the default assistant at one extreme.
...
4. **Go/No-Go (per model):** a role is *viable* if ≥9/200 responses score 3 (≥4.5%, which projects to ≥27 score-3 at the full-run scale of 600 responses/role, meeting the `--min_count 25` used in Phase 2). 
   ...
   - **NO-GO** if <30% viable → do NOT burn the night on 90 roles. Instead: (a) try the user-turn-concatenation fix above if not already applied; (b) try a variant where score-2 responses ("identifies as AI but exhibits role attributes") are also included in step 4 — this needs a small documented patch; (c) run a reduced 45-role version (drop half of each stratum) with whichever variant looks best. A model that *can't* leave the assistant persona is itself an interesting SPP result — document it with examples rather than forcing the pipeline.

## Phase 2 — Full overnight run (90 personas)

1. **Use the canonical 90-role list committed at the sprint repo root: `roles_90.json`** (30 helpers / 30 humans / 30 non-human; shared verbatim with the Gemma anchors track — do not re-derive or substitute). Copy it to `/workspace/exp/roles_90.json` as the run record. `default` is used in addition to the 90 and is not one of them. (Only if your run already selected a different list before this instruction existed: keep yours for consistency and note the divergence prominently in RESULTS.md and COMPARISON.md.)
2. Write a driver script `overnight.sh` that loops over the **priority queue of gate-passing models** (same 90 roles for every model), with `set -e`, per-step logging to `/workspace/exp/<model>/logs/`, and the `--roles $(cat ...)` selection. Per model:
   - Step 1 (generation, all 90 roles + default, **`--question_count 120`** — 600 responses/role, uniform across models so they're comparable, sized so ~2–3 models finish per night) → then step 2 and step 3 **in parallel** (backgrounded with wait) → step 4 (**`--min_count 25`**) → step 5 → per-model analysis script (below).
   ...
   - After each model completes (not only at the very end): back up results off the ephemeral pod — upload `/workspace/exp/<model>/` (vectors, `axis.pt`, PCA outputs, plots, reports, `roles_90.json`, logs; **exclude** raw activations and responses, ~25GB+) to a **private** HF dataset repo `timf34/spp-assistant-axis-results` via `huggingface_hub.upload_folder` using `HF_TOKEN` from the `.env`. Create the repo private if it doesn't exist.
...
5. Rough budget check before launch: per model, 91 roles × 600 responses ≈ 55k generations of ≤512 tokens (~2–4h on one H100 for a 3B model) plus ~55k `openai/gpt-4.1-mini` judge calls via OpenRouter (~$8–15/model, ~$40–75 for all five; step 3 is restartable if it dies, and OpenRouter rate limits surface as ordinary API errors — the script's rerun-on-malformed behavior covers them, so rerun step 3 once more per model as the repo README recommends). Use your measured Phase 1 throughput to project the night honestly in STATE.md: how many models should complete by 8am. If it projects to fewer than 2, cut `--question_count` to 60 and `--min_count` to 12 — uniformly, for every model — and note the deviation.
