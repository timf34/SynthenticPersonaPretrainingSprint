...
1. **Use the canonical 60-role list committed at the sprint repo root: `roles_60.json`** (20 helpers / 20 humans / 20 non-human; shared verbatim with the Gemma anchors track — do not re-derive or substitute). Copy it to `/workspace/exp/roles_60.json` as the run record. `default` is used in addition to the 60 and is not one of them. (Only if your run already selected a different list before this instruction existed: keep yours for consistency and note the divergence prominently in RESULTS.md and COMPARISON.md.)
